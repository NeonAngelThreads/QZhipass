...
async function submitPhone() {
  ...
  try { await authStore.passwordLogin(phoneForm.phone.trim(), phoneForm.password); await finishLogin() }
  catch (error) { showLoginError(error, '账号或密码错误') }
  finally { submitting.value = false }
}
async function submitEmail() {
  if (!canSubmitEmail.value) return
  submitting.value = true; message.value = ''
  try { await authStore.passwordLogin(emailForm.email.trim(), emailForm.password); await finishLogin() }
  catch (error) { showLoginError(error, '账号或密码错误') }
  finally { submitting.value = false }
}
async function submitSms() {
  ...
  catch (error) { showLoginError(error, '手机号或验证码错误') }
  finally { submitting.value = false }
}
...
<template>
  <main class="login-page">
    <section class="brand-panel">
      <div class="brand-content"><div class="brand-mark">企</div><p class="eyebrow">企业智能工作空间</p><h1>企智通</h1><p class="slogan">让企业知识、智能体与每一次协作自然连接。</p><div class="brand-tags"><span>安全访问</span><span>统一智能体</span><span>知识协作</span></div></div>
    </section>

...
const canSubmitPhone = computed(
  () => isValidMobile(phoneForm.phone.trim()) && Boolean(phoneForm.password) && !submitting.value
)
const canSubmitEmail = computed(
  () => isValidEmail(emailForm.email.trim()) && Boolean(emailForm.password) && !submitting.value
)
const canSendSms = computed(
  () => isValidMobile(smsForm.phone.trim()) && countdown.value === 0 && !smsSending.value
)
const canSubmitSms = computed(
  () => isValidMobile(smsForm.phone.trim()) && /^\d{6}$/.test(smsForm.code.trim()) && !submitting.value
)
...
function showLoginError(error: unknown, fallback: string) {
  try {
    ...
    showMessage(errorText(error, fallback))
  }
}
...
async function submitPhone() {
  ...
  message.value = ''
  try {
    await authStore.passwordLogin(phoneForm.phone.trim(), phoneForm.password)
    await finishLogin()
  } catch (error) {
    showLoginError(error, '手机号或密码错误')
  } finally {
...
async function submitEmail() {
  ...
  message.value = ''
  try {
    await authStore.passwordLogin(emailForm.email.trim(), emailForm.password)
    await finishLogin()
  } catch (error) {
    showLoginError(error, '邮箱或密码错误')
  } finally {
    submitting.value = false
  }
}
...
async function submitSms() {
  ...
  message.value = ''
  try {
    await authStore.smsLogin(smsForm.phone.trim(), smsForm.code.trim())
    await finishLogin()
  } catch (error) {
    showLoginError(error, '手机号或验证码错误')
  } finally {
    submitting.value = false
  }
}
...
async function requestSmsCode() {
  ...
  try {
    ...
    showMessage(errorText(error, '验证码发送失败'))
  } finally {
  ...
  <main class="login-page">
    <section class="brand-panel">
      <div class="brand-content">
        ...
        <p class="eyebrow">企业智能工作空间</p>
        <h1>企智通</h1>
        <p class="slogan">让企业知识、智能体与每一次协作自然连接。</p>
    ...
    <section class="form-side">
      <div class="login-card">
        <header>
          <span class="mini-brand">企智通</span>
          <h2>欢迎回来</h2>
          <p>请选择登录方式进入企业智能工作台</p>
        </header>
        <div class="tabs" role="tablist">
          <button
            :class="{ active: activeTab === 'phone' }"
            type="button"
            @click="activeTab = 'phone'; message = ''"
          >
            手机密码
          </button>
          <button
        ...
        <form v-if="activeTab === 'phone'" @submit.prevent="submitPhone">
          ...
          <label>
            ...
            <span class="password-field">
              ...
              />
              <button type="button" @click="showPhonePassword = !showPhonePassword">
                {{ showPhonePassword ? '隐藏' : '显示' }}
              </button>
            </span>
          </label>
          <button class="primary" :disabled="!canSubmitPhone" type="submit">
            {{ submitting ? '登录中…' : '登录' }}
          </button>
        </form>
        <form v-else-if="activeTab === 'email'" @submit.prevent="submitEmail">
          ...
          <button class="primary" :disabled="!canSubmitEmail" type="submit">
            {{ submitting ? '登录中…' : '登录' }}
          </button>
        </form>
          ...
          <label>
            ...
            <span class="sms-field">
              <input
                ...
                placeholder="六位验证码"
              />
              <button type="button" :disabled="!canSendSms" @click="requestSmsCode">
                {{ smsSending ? '发送中…' : countdown ? `${countdown}s` : '发送验证码' }}
              </button>
            </span>
          </label>
          <button class="primary" :disabled="!canSubmitSms" type="submit">
            {{ submitting ? '登录中…' : '登录' }}
          </button>
        </form>

...
async function doDeactivate() {
  ...
  try {
    ...
    ElMessage.error(getErrorMessage(error, '注销失败，请稍后重试'))
  }
}
    ...
    <div class="main-area">
      ...
      <div class="content">
        ...
        <div class="filter-bar">
          <el-input v-model="searchKeyword" placeholder="搜索姓名 / 手机号" :prefix-icon="Search" clearable @input="handleSearch" />
          <button class="secondary-button" @click="fetchUsers">
        ...
        <div class="workspace-card">
          <div class="section-tabs">
            <button class="active">全部用户<span>{{ totalFiltered }}</span></button>
          </div>

          <div class="table-wrap" v-loading="loading">
            <el-table :data="paginatedUsers" stripe empty-text="暂无用户数据" style="width: 100%">
              ...
              <el-table-column prop="lastLoginAt" label="最后登录" min-width="160">
                <template #default="{ row }">
                  <span class="time-cell">{{ row.lastLoginAt || '-' }}</span>
                </template>
              </el-table-column>
              <el-table-column label="操作" width="120" align="center" fixed="right">
                <template #default="{ row }">
                  ...
                  <template v-else>
                    <button class="link-button danger" @click="openDeactivateModal(row)">注销</button>
                  </template>
                </template>
              </el-table-column>
            </el-table>
          ...
          <div style="padding: 16px 20px">
            <div class="deactivate-body">
              ...
              <p class="deactivate-text">
                您确定要注销用户【{{ deactivateTarget?.name }}】吗？注销后该账号将立即失效，用户将无法登录系统，且该操作不可撤销。
              </p>
            </div>
